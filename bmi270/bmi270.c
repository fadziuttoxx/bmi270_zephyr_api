...
/* NOMO_1.duration conversion is 20 ms / LSB */
#define NOMO_1_DURATION_MSEC_TO_LSB(_ms)	\
	BMI270_NOMO_1_DURATION(_ms / 20)

static int bmi270_write_nomo_threshold(const struct device *dev,
				       struct sensor_value val)
{
	struct bmi270_data *data = dev->data;
	...
	/* this takes configuration in g. */
	if (val.val1 > 0) {
		LOG_DBG("nomo_threshold set to max");
		val.val2 = 1e6;
	}

	/* max = BIT_MASK(10) = 1g => 0.49 mg/LSB */
	uint16_t lsbs = (val.val2 * BMI270_NOMO_2_THRESHOLD_MASK) / 1e6;
	...
	uint16_t nomo_2 = BMI270_NOMO_2_THRESHOLD(lsbs)
		| BMI270_NOMO_2_OUT_CONF_BIT_5;

	data->nomo_2 = nomo_2;
	return 0;
}

static int bmi270_write_nomo_duration(const struct device *dev, uint32_t ms)
{
	struct bmi270_data *data = dev->data;
	uint16_t val = NOMO_1_DURATION_MSEC_TO_LSB(ms)
		| BMI270_NOMO_1_SELECT_XYZ;
			   ...
			   enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((chan == SENSOR_CHAN_ACCEL_X) || (chan == SENSOR_CHAN_ACCEL_Y)
		...
		case SENSOR_ATTR_SLOPE_DUR_nomo:
			return bmi270_write_nomo_duration(dev, val->val1);
		case SENSOR_ATTR_SLOPE_TH_nomo:
			return bmi270_write_nomo_threshold(dev, *val);
#endif
		default:
			ret = -ENOTSUP;
		}
...
static const struct bmi270_feature_config bmi270_feature_base = {
	...
	.nomo_1 = &(struct bmi270_feature_reg){ .page = 2, .addr = 0x30 },
	.nomo_2 = &(struct bmi270_feature_reg){ .page = 2, .addr = 0x32 },
};
