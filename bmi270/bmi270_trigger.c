...
static void bmi270_thread_cb(const struct device *dev)
{
	...
	if (atomic_test_and_clear_bit(&data->int_flags, INT_FLAGS_INT1)) {
		...
		if (data->nomotion_handler != NULL) {
			if (int_status & BMI270_INT_STATUS_NO_MOTION) {
				data->nomotion_handler(dev, data->nomotion_trigger);
			}
		}
...
int bmi270_feature_reg_write(const struct device *dev, const struct bmi270_feature_reg *reg,
			     uint16_t value)
{
	int ret;
...
int bmi270_feature_reg_read(const struct device *dev, const struct bmi270_feature_reg *reg,
			    uint16_t *value)
{
	int ret;
	uint8_t feat_page = reg->page;
...
static int bmi270_anymo_config(const struct device *dev, bool enable)
{
	...
	uint8_t int1_map_feat;

	ret = bmi270_reg_read(dev, BMI270_REG_INT1_MAP_FEAT, &int1_map_feat, 1);
	if (ret < 0) {
		LOG_ERR("failed reading INT1_MAP_FEAT (%d)", ret);
		return ret;
	}
...
static int bmi270_nomo_config(const struct device *dev, bool enable)
{
	const struct bmi270_config *cfg = dev->config;
	struct bmi270_data *data = dev->data;
	uint16_t nomo_2;
	...
	if (enable) {
		ret = bmi270_feature_reg_write(dev, cfg->feature->nomo_1, data->nomo_1);
		if (ret < 0) {
			return ret;
		}
	}
	...
	nomo_2 = data->nomo_2;
	if (enable) {
		nomo_2 |= BMI270_NOMO_2_ENABLE;
	}

	ret = bmi270_feature_reg_write(dev, cfg->feature->nomo_2, nomo_2);
	if (ret < 0) {
		return ret;
	}
	...
	if (enable) {
		int1_map_feat |= BMI270_INT_MAP_NO_MOTION;
	}
		       ...
		       sensor_trigger_handler_t handler)
{
	...
	case SENSOR_TRIG_STATIONARY:
		...
		k_mutex_lock(&data->trigger_mutex, K_FOREVER);
		data->nomotion_handler = handler;
		data->nomotion_trigger = trig;
		k_mutex_unlock(&data->trigger_mutex);
		return bmi270_nomo_config(dev, handler != NULL);
